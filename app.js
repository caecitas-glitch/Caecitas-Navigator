/**
 * Navis - Star Citizen Fleet Logistics & Starmap Route Optimizer
 * Dynamic multi-hop route solver, multi-ship hauler & flight planner.
 */
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  if (data.ships) {
    data.ships.forEach(ship => {
      ...
      opt.value = ship.id;
      const hSize = ship.hangarSize ? ` [${ship.hangarSize}]` : '';
      opt.textContent = `${ship.name} (${ship.scu > 0 ? ship.scu.toLocaleString() + ' SCU' : 'Custom'}${hSize})`;
      ...
      card.innerHTML = `
        <div class="flex items-center justify-between">
          <div class="flex items-center gap-1.5 flex-wrap">
            ...
            <span class="hangar-badge hangar-badge-${(ship.hangarSize || 'medium').toLowerCase()}">${ship.hangarSize || 'Medium'} Hangar</span>
  ...
  btnAddFleetShip.addEventListener("click", () => {
    ...
    const newShip = {
      ...
      hangarSize: shipTemplate.hangarSize || "Medium",
  ...
  // ==========================================
  const SAVED_ROUTES_KEY = "navis_saved_routes";
  if (!localStorage.getItem(SAVED_ROUTES_KEY) && localStorage.getItem("caecitas_saved_navigator_routes")) {
    try {
      localStorage.setItem(SAVED_ROUTES_KEY, localStorage.getItem("caecitas_saved_navigator_routes"));
    } catch (e) {}
  }
  ...
  // ==========================================
  // USEFUL INFORMATION HUB (CARGO GRIDS, ELEVATORS & BUGS)
  // ==========================================
  const btnInfoHub = document.getElementById("btn-info-hub");
  const btnCloseInfoHub = document.getElementById("btn-close-info-hub");
  const infoHubModal = document.getElementById("info-hub-modal");
  ...
  const gridMfgButtons = document.querySelectorAll(".mfg-btn");
  const activeGridGuideImg = document.getElementById("active-grid-guide-img");

  if (btnInfoHub && infoHubModal) {
    btnInfoHub.addEventListener("click", () => {
      infoHubModal.classList.remove("hidden");
    });
  }

  if (btnCloseInfoHub && infoHubModal) {
    btnCloseInfoHub.addEventListener("click", () => {
      infoHubModal.classList.add("hidden");
    });
  }

  if (infoHubModal) {
    infoHubModal.addEventListener("click", (e) => {
  ...
  infoTabButtons.forEach(btn => {
    btn.addEventListener("click", () => {
      ...
      btn.classList.add("active");
      const targetTab = document.getElementById(`info-tab-${btn.dataset.tab}`);
      if (targetTab) targetTab.classList.add("active");
    });
  });

  gridMfgButtons.forEach(btn => {
    btn.addEventListener("click", () => {
      gridMfgButtons.forEach(b => b.classList.remove("active"));
      btn.classList.add("active");
      if (activeGridGuideImg && btn.dataset.img) {
        activeGridGuideImg.src = btn.dataset.img;
        activeGridGuideImg.onerror = function() {
          if (!this.dataset.fb) {
            this.dataset.fb = '1';
            this.src = `assets/guides/${btn.dataset.img}`;
          }
        };
      }
    });
